...
- **[019. Go 文件系统：从字节到文件的完整旅程](notes/019-file-system.md)**
  - 文件本质与字节、os.ReadFile 一次性读取、流式读取与 bufio、文件元信息、写入与追加、Flush 刷新缓冲、Close 关闭资源

## 📦 阶段五：包与模块

> 代码组织、模块管理、第三方包

- **[020. Go 包（Package）：代码组织的基本单元](notes/020-packages.md)**
  - 模块与 go.mod、包的概念、创建和使用包、导出规则（大小写）、第三方包
